...
function(formula, data, clustervar = NULL, robust = TRUE){
  ...
  if(is.null(clustervar) & robust == FALSE){gmm1 = gmm(frm1, frm2, data = data.new, vcov="iid")}
  if(is.null(clustervar) & robust == TRUE){gmm1 = gmm(frm1, frm2, data = data.new, vcov="HAC")}
  if(!is.null(clustervar)){gmm1 = suppressMessages(gmmcl(frm1, frm2, data = data.new, cluster = clustervar))}
